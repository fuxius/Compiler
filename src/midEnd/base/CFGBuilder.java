...
//这个CFG构建器的总体思路是：
//首先构建基本的控制流图(CFG)，然后基于CFG计算支配关系并构建支配树，最后计算支配边界。具体步骤如下：
//
//CFG构建阶段：
//
//
//分析每个基本块的最后一条指令，根据指令类型(条件分支或无条件分支)建立基本块之间的前驱后继关系
//对于条件分支，建立当前块到then块和else块的边
//对于无条件分支，建立当前块到目标块的边
//
//
//支配关系计算阶段：
//
//
//从入口基本块开始，对每个基本块B，如果从入口到另一个块Y的所有路径都必须经过B，则B支配Y
//使用可达性分析来确定支配关系：如果去掉某个块后，入口无法到达另一个块，则第一个块支配第二个块
//构建支配树：对每个被支配的块，在其所有支配者中找到最接近的一个作为直接支配者
//
//
//支配边界计算阶段：
//
//
//对于每个支配者节点D，遍历它支配的所有节点N
//检查N的每个后继S，如果D不支配S，则S在D的支配边界中
//额外检查D自己的后继节点，如果不被D支配也要加入支配边界
//
//这些分析结果对后续的代码优化非常重要：
//
//CFG用于确定程序的控制流程，是大多数优化的基础
//支配关系用于确定代码的嵌套结构，对循环优化很重要
//支配边界用于放置Φ函数，这对SSA形式的构建是必需的
//
//整个过程采用了自底向上的设计方法，每一步都基于前面步骤的结果，最终构建出完整的程序分析框架。